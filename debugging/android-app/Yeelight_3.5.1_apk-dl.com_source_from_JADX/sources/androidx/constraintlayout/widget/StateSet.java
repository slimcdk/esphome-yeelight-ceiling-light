...
public class StateSet {
    ...
    static class State {
        ...
        public State(Context context, XmlPullParser xmlPullParser) {
            ...
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), C0218R.styleable.State);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0218R.styleable.State_android_id) {
                    this.mId = obtainStyledAttributes.getResourceId(index, this.mId);
                } else if (index == C0218R.styleable.State_constraints) {
    ...
    static class Variant {
        ...
        public Variant(Context context, XmlPullParser xmlPullParser) {
            this.mIsLayout = false;
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), C0218R.styleable.Variant);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0218R.styleable.Variant_constraints) {
                    this.mConstraintID = obtainStyledAttributes.getResourceId(index, this.mConstraintID);
                    String resourceTypeName = context.getResources().getResourceTypeName(this.mConstraintID);
                    context.getResources().getResourceName(this.mConstraintID);
                    if ("layout".equals(resourceTypeName)) {
                ...
                } else if (index == C0218R.styleable.Variant_region_heightLessThan) {
                    this.mMaxHeight = obtainStyledAttributes.getDimension(index, this.mMaxHeight);
                } else if (index == C0218R.styleable.Variant_region_heightMoreThan) {
                    this.mMinHeight = obtainStyledAttributes.getDimension(index, this.mMinHeight);
                } else if (index == C0218R.styleable.Variant_region_widthLessThan) {
                    this.mMaxWidth = obtainStyledAttributes.getDimension(index, this.mMaxWidth);
                } else if (index == C0218R.styleable.Variant_region_widthMoreThan) {
    ...
    private void load(android.content.Context r10, org.xmlpull.v1.XmlPullParser r11) {
        /*
            ...
            int[] r1 = androidx.constraintlayout.widget.C0218R.styleable.StateSet
        ...
        L_0x0010:
            ...
            int r5 = androidx.constraintlayout.widget.C0218R.styleable.StateSet_defaultState
    ...
    public int updateConstraints(int i, int i2, float f, float f2) {
        int findMatch;
        if (i == i2) {
            State state = (State) (i2 == -1 ? this.mStateList.valueAt(0) : this.mStateList.get(this.mCurrentStateId));
            if (state == null) {
                return -1;
            }
