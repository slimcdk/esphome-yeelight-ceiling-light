...
public enum BleMessage {
    ...
    TELINK_STATUS_NOTIFICATION(Opcode.BLE_GATT_OP_CTRL_DB.name()),
    UNKNOWN("0000");
    
    public static final String UNKNOWN_MESSAGE = "UNKNOWN_MESSAGE";
    public String value;

    private BleMessage(String str) {
        this.value = "";
        this.value = str;
    }

    public static BleMessage parseBleMessage(String str) {
        for (BleMessage bleMessage : values()) {
            if (bleMessage.value.equals(str)) {
                return bleMessage;
            }
        }
        return UNKNOWN;
