...
public class ScanBarcodeActivity extends Activity implements SurfaceHolder.Callback {
    ...
    private C2563g lastResult;
    private int mFrom;
    private String mModel;
    /* access modifiers changed from: private */
    public ObjectAnimator mScanAnim;
    private View mScanLine;
    private C2563g savedResultToShow;
    private ViewfinderView viewfinderView;

    private void decodeOrStoreSavedBitmap(Bitmap bitmap, C2563g gVar) {
        ...
        C2563g gVar2 = this.savedResultToShow;
        if (gVar2 != null) {
            this.handler.sendMessage(Message.obtain(captureActivityHandler, C2875R.C2877id.decode_succeeded, gVar2));
        }
    ...
    private void displayFrameworkBugMessageAndExit() {
        ...
        builder.setMessage(getString(C2875R.string.msg_camera_framework_bug));
        builder.setPositiveButton(C2875R.string.confirm, new FinishListener(this));
        builder.setOnCancelListener(new FinishListener(this));
        builder.show();
    }

    private static void drawLine(Canvas canvas, Paint paint, C2564h hVar, C2564h hVar2, float f) {
        if (hVar != null && hVar2 != null) {
            canvas.drawLine(f * hVar.mo19546c(), f * hVar.mo19547d(), f * hVar2.mo19546c(), f * hVar2.mo19547d(), paint);
        }
    }

    private void drawResultPoints(Bitmap bitmap, float f, C2563g gVar) {
        C2564h hVar;
        C2564h hVar2;
        C2564h[] e = gVar.mo19541e();
        if (e != null && e.length > 0) {
            Canvas canvas = new Canvas(bitmap);
            Paint paint = new Paint();
            paint.setColor(getResources().getColor(C2875R.color.result_points));
            if (e.length == 2) {
                paint.setStrokeWidth(4.0f);
                hVar = e[0];
                hVar2 = e[1];
            } else if (e.length == 4 && (gVar.mo19538b() == BarcodeFormat.UPC_A || gVar.mo19538b() == BarcodeFormat.EAN_13)) {
                ...
                for (C2564h hVar3 : e) {
                    if (hVar3 != null) {
                        canvas.drawPoint(hVar3.mo19546c() * f, hVar3.mo19547d() * f, paint);
                    }
                }
                return;
            }
            drawLine(canvas, paint, hVar, hVar2, f);
        }
    }
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        if (!this.isInit) {
            ...
            if (surfaceHolder == null) {
                ...
                try {
                    int dimensionPixelSize = getResources().getDimensionPixelSize(C2875R.dimen.std_titlebar_height);
                    ...
                    if (this.handler == null) {
                        ...
                        this.handler = new CaptureActivityHandler(this, arrayList, (Map<DecodeHintType, ?>) null, (String) null, this.cameraManager);
                    }
                    decodeOrStoreSavedBitmap((Bitmap) null, (C2563g) null);
        ...
        if (handler2 != null) {
            ...
            handler2.postDelayed(new Runnable() {
                public void run() {
                    ...
                    Toast.makeText(scanBarcodeActivity, scanBarcodeActivity.getString(C2875R.string.verifying_qrcode_failed), 0).show();
                }
            }, 1000);
        }
    }
    ...
    public void handleDecode(C2563g gVar, Bitmap bitmap, float f) {
        this.inactivityTimer.onActivity();
        this.lastResult = gVar;
        if (bitmap != null) {
            this.beepManager.playBeepSoundAndVibrate();
            drawResultPoints(bitmap, f, gVar);
        }
        this.mScanAnim.cancel();
        if (this.mFrom != 0) {
            verifyQrcode(gVar.mo19542f());
            return;
        }
        Intent intent = new Intent();
        intent.putExtra(SCAN_RESULT, gVar.mo19542f());
        setResult(-1, intent);
        finish();
    }
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(C2875R.layout.scan_barcode_activity);
        findViewById(C2875R.C2877id.module_a_3_return_btn).setOnClickListener(new View.OnClickListener() {
        ...
        ((TextView) findViewById(C2875R.C2877id.module_a_3_return_title)).setText(C2875R.string.scan_qrcode_add_device);
        if (this.mFrom == 200) {
            findViewById(C2875R.C2877id.goto_support_qrcode_page).setVisibility(8);
        }
        if (!TextUtils.isEmpty(stringExtra)) {
            ((TextView) findViewById(C2875R.C2877id.tips)).setText(stringExtra);
        }
        this.mScanLine = findViewById(C2875R.C2877id.scan_line);
    ...
    public void onPause() {
        ...
        if (!this.hasSurface) {
            ((SurfaceView) findViewById(C2875R.C2877id.preview_view)).getHolder().removeCallback(this);
        }
        this.mScanAnim.cancel();
        this.hasSurface = false;
        this.isInit = false;
    ...
    public void onResume() {
        ...
        if (this.cameraManager == null) {
            ...
            ViewfinderView viewfinderView2 = (ViewfinderView) findViewById(C2875R.C2877id.viewfinder_view);
        ...
        SurfaceHolder holder = ((SurfaceView) findViewById(C2875R.C2877id.preview_view)).getHolder();
        if (this.hasSurface) {
            initCamera(holder);
        } else {
    ...
    public void restartPreviewAfterDelay(long j) {
        CaptureActivityHandler captureActivityHandler = this.handler;
        if (captureActivityHandler != null) {
            captureActivityHandler.sendEmptyMessageDelayed(C2875R.C2877id.restart_preview, j);
        }
        resetStatusView();
    }
