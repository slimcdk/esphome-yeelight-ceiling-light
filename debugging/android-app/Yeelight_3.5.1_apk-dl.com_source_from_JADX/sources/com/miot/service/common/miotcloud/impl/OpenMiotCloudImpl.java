...
public class OpenMiotCloudImpl implements MiotCloud {
    ...
    private void addHeadForBody(C4297n nVar) {
        if (nVar == null) {
            Logger.m7500e(TAG, "builder is null");
            return;
        }
        AppConfiguration appConfig = ServiceManager.getInstance().getAppConfig();
        if (appConfig == null) {
            Logger.m7500e(TAG, "appConfig is null");
            return;
        }
        long longValue = appConfig.getAppId().longValue();
        if (longValue == 0) {
            Logger.m7500e(TAG, "appId is null");
            return;
        }
        String accessToken = this.mPeople.getAccessToken();
        if (accessToken == null) {
            Logger.m7500e(TAG, "accessToken is null");
            return;
        }
        nVar.mo27923a("clientId", Long.toString(longValue));
        nVar.mo27923a("accessToken", accessToken);
        if (this.mPeople.getMacKey() != null) {
            nVar.mo27923a("macKey", this.mPeople.getMacKey());
        }
        if (this.mPeople.getMacAlgorithm() != null) {
            nVar.mo27923a("macAlgorithm", this.mPeople.getMacAlgorithm());
        }
        nVar.mo27923a("locale", Locale.getDefault().toString().replace("_", Constants.ACCEPT_TIME_SEPARATOR_SERVER));
    }

    private C4315w genRequestBody(List<NameValuePair> list) {
    ...
    public int post(final String str, List<NameValuePair> list, final MiotCloud.ResponseHandler responseHandler) {
        ...
        OkHttpManager.getInstance().enqueue(new C4312v.C4314b().mo28055o(str2).mo28052l(genRequestBody).mo28047g(), new HttpCallback<String>() {
    ...
    public HttpResponse post(String str, List<NameValuePair> list) {
        ...
        if (people == null) {
            ...
            if (genRequestBody == null) {
                ...
                try {
                    C4318x execute = OkHttpManager.getInstance().execute(new C4312v.C4314b().mo28055o(str3).mo28052l(genRequestBody).mo28047g());
                    if (!execute.mo28065t()) {
                        i = execute.mo28060o();
                        exc = execute.mo28067u();
                    } else {
                        String n = execute.mo28056k().mo28089n();
                        if (ServiceManager.getInstance().isDemoApp()) {
                            n = specialClientIdDecrypt(n);
                        }
                        MyLogger.getInstance().log(TAG, "Post  request:" + str4 + "  response:" + n);
