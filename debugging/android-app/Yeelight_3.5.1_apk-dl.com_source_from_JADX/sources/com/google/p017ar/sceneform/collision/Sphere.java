...
public class Sphere extends CollisionShape {
    ...
    public void transform(TransformProvider transformProvider, CollisionShape collisionShape) {
        ...
        if (collisionShape instanceof Sphere) {
            ...
            sphere.radius = this.radius * Math.max(Math.abs(Math.min(Math.min(vector3.f3277x, vector3.f3278y), vector3.f3279z)), Math.max(Math.max(vector3.f3277x, vector3.f3278y), vector3.f3279z));
        }
    }
}
