...
public final class AndroidHeapDumper implements HeapDumper {
    ...
    private void showToast(final FutureResult<Toast> futureResult) {
        this.mainHandler.post(new Runnable() {
            public void run() {
                ...
                toast.setDuration(1);
                toast.setView(LayoutInflater.from(AndroidHeapDumper.this.context).inflate(C4247R.layout.leak_canary_heap_dump_toast, (ViewGroup) null));
                toast.show();
        ...
        LeakCanaryInternals.executeOnFileIoThread(new Runnable() {
            public void run() {
                if (!AndroidHeapDumper.this.leakDirectoryProvider.isLeakStorageWritable()) {
                    CanaryLog.m11817d("Could not attempt cleanup, leak storage not writable.", new Object[0]);
                    return;
                }
                File heapDumpFile = AndroidHeapDumper.this.getHeapDumpFile();
                if (heapDumpFile.exists()) {
                    CanaryLog.m11817d("Previous analysis did not complete correctly, cleaning: %s", heapDumpFile);
                    if (!heapDumpFile.delete()) {
                        CanaryLog.m11817d("Could not delete file %s", heapDumpFile.getPath());
                    }
                }
            }
        });
    ...
    public File dumpHeap() {
        if (!this.leakDirectoryProvider.isLeakStorageWritable()) {
            CanaryLog.m11817d("Could not write to leak storage to dump heap.", new Object[0]);
            this.leakDirectoryProvider.requestWritePermission();
        } else {
            File heapDumpFile = getHeapDumpFile();
            try {
                if (!heapDumpFile.createNewFile()) {
                    CanaryLog.m11817d("Could not dump heap, previous analysis still is in progress.", new Object[0]);
                } else {
                    FutureResult futureResult = new FutureResult();
                    showToast(futureResult);
                    if (!futureResult.wait(5, TimeUnit.SECONDS)) {
                        CanaryLog.m11817d("Did not dump heap, too much time waiting for Toast.", new Object[0]);
                    } else {
                        ...
                        try {
                            ...
                            cleanup();
                            CanaryLog.m11818d(e, "Could not perform heap dump", new Object[0]);
                        }
                    }
                }
            } catch (IOException e2) {
                cleanup();
                CanaryLog.m11818d(e2, "Could not check if heap dump file exists", new Object[0]);
            }
        }
