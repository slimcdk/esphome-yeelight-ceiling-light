...
final class DisplayLeakAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (getItemViewType(i) == 0) {
            if (view == null) {
                view = LayoutInflater.from(context).inflate(C4247R.layout.leak_canary_ref_top_row, viewGroup, false);
            }
            ((TextView) findById(view, C4247R.C4249id.__leak_canary_row_text)).setText(context.getPackageName());
        } else {
            if (view == null) {
                view = LayoutInflater.from(context).inflate(C4247R.layout.leak_canary_ref_row, viewGroup, false);
            }
            TextView textView = (TextView) findById(view, C4247R.C4249id.__leak_canary_row_text);
            boolean z2 = i == 1;
            if (i == getCount() - 1) {
                z = true;
            }
            ...
            ((DisplayLeakConnectorView) findById(view, C4247R.C4249id.__leak_canary_row_connector)).setType(z2 ? DisplayLeakConnectorView.Type.START : z ? DisplayLeakConnectorView.Type.END : DisplayLeakConnectorView.Type.NODE);
            ((MoreDetailsView) findById(view, C4247R.C4249id.__leak_canary_row_more)).setOpened(this.opened[i]);
        }
        return view;
    }
