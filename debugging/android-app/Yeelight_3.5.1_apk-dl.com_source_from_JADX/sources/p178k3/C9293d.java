package p178k3;

import android.content.Context;
import android.content.pm.PackageManager;
import android.content.pm.Signature;
import p181l3.C9523b;

/* renamed from: k3.d */
class C9293d {
    /* renamed from: a */
    public static boolean m22737a(Context context, Signature[] signatureArr, boolean z) {
        String str;
        if (!z) {
            str = "ignore wechat app signature validation";
        } else {
            int length = signatureArr.length;
            int i = 0;
            while (i < length) {
                String lowerCase = signatureArr[i].toCharsString().toLowerCase();
                C9523b.m23171a("MicroMsg.SDK.WXMsgImplComm", "check signature:" + lowerCase);
                if (lowerCase.equals("")) {
                    str = "pass";
                } else {
                    i++;
                }
            }
            return false;
        }
        C9523b.m23171a("MicroMsg.SDK.WXMsgImplComm", str);
        return true;
    }

    /* renamed from: b */
    public static boolean m22738b(Context context, String str, boolean z) {
        if (!z) {
            C9523b.m23171a("MicroMsg.SDK.WXMsgImplComm", "ignore wechat app signature validation");
            return true;
        }
        try {
            return m22737a(context, context.getPackageManager().getPackageInfo(str, 64).signatures, z);
        } catch (PackageManager.NameNotFoundException unused) {
            return false;
        }
    }
}
