...
public class BleStandardAuthLoginConnector extends BleStandardAuthConnector {
    ...
    public void processStep1Plus() {
        ...
        BluetoothLog.m10338v("BleStandardAuthLoginConnector Process Step 1 plus ...");
        openAuthNotify(new BleNotifyResponse() {
            public void onNotify(UUID uuid, UUID uuid2, byte[] bArr) {
            }

            public void onResponse(int i) {
                BluetoothLog.m10338v("BleStandardAuthLoginConnector Step 1 plus onResponse: " + Code.toString(i));
                if (i == 0) {
    ...
    private void receiveDeviceResult(byte[] bArr) {
        ...
        BluetoothLog.m10338v("BleStandardAuthLoginConnector receiveDeviceResult ..., value = " + ByteUtils.byteToString(bArr));
    ...
    private void recvDevConfirmation(byte[] bArr) {
        BluetoothLog.m10338v("BleStandardAuthLoginConnector recvDevConfirmation ...");
        if (isCanceled()) {
            dispatchResult(-2);
            return;
        }
        ...
        if (ByteUtils.equals(bArr, SecurityChipUtil.sha256HMAC(this.mDevKey, bArr3))) {
            BluetoothLog.m10338v("BleStandardAuthLoginConnector verify device confirm success ...");
            sendAppConfirm();
            this.mHandler.removeMessages(MSG_NOTIFY_TIMEOUT);
    ...
    private void recvDevRandom(byte[] bArr) {
        ...
        BluetoothLog.m10334d("BleStandardAuthLoginConnector recvDevRandom ...");
        this.mHandler.removeMessages(MSG_NOTIFY_TIMEOUT);
    ...
    private void sendAppConfirm() {
        int i;
        BluetoothLog.m10338v("BleStandardAuthLoginConnector sendAppConfirm ...");
        if (isCanceled()) {
            i = -2;
        } else if (ByteUtils.isEmpty(this.mAppConfirm)) {
            BluetoothLog.m10338v("BleStandardAuthLoginConnector sendAppConfirm mAppConfirm is empty");
            i = -52;
            ...
            public void onResponse(int i, Bundle bundle) {
                BluetoothLog.m10338v("BleStandardAuthLoginConnector sendAppConfirm onResponse: " + Code.toString(i));
                if (i == 0) {
    ...
    public void sendAppRandom() {
        ...
        BluetoothLog.m10338v("BleStandardAuthLoginConnector sendAppRandom ...");
        ...
        if (!writeChannel(generateAppRandom, 11, new IResponse() {
            ...
            public void onResponse(int i, Bundle bundle) {
                BluetoothLog.m10338v("BleStandardAuthLoginConnector sendAppPublicKey onResponse: " + Code.toString(i));
                if (i == 0) {
                    BleStandardAuthLoginConnector.this.mHandler.removeMessages(BleStandardAuthLoginConnector.MSG_NOTIFY_TIMEOUT);
                    BleStandardAuthLoginConnector.this.mHandler.sendEmptyMessageDelayed(BleStandardAuthLoginConnector.MSG_NOTIFY_TIMEOUT, 10000);
                    return;
    ...
    public void sendLoginStart() {
        ...
        BluetoothLog.m10338v("BleStandardAuthLoginConnector sendLoginStart ...");
        MiotBleClient.getInstance().writeNoRsp(getMac(), BluetoothConstants.MISERVICE, BluetoothConstants.CHARACTER_AUTH, LOGIN_START_W_RANDOM, new BleWriteResponse() {
            public void onResponse(int i) {
                BluetoothLog.m10338v("BleStandardAuthLoginConnector sendRegStart onResponse: " + Code.toString(i));
                if (i == 0) {
    ...
    public void processHandlerMessage(Message message) {
        if (message.what == MSG_NOTIFY_TIMEOUT) {
            BluetoothLog.m10339w("BleStandardAuthLoginConnector notify timeout");
            dispatchResult(-7);
        }
    }
    ...
    public void processStep1() {
        ...
        BluetoothLog.m10338v("BleStandardAuthLoginConnector Process Step 1 ...");
        ...
        openStandardAuthNotify(new BleNotifyResponse() {
            ...
            public void onResponse(int i) {
                BluetoothLog.m10338v("BleStandardAuthLoginConnector Step 1 onResponse: " + Code.toString(i));
                if (i == 0) {
