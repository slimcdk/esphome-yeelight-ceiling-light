...
public class DisplayLeakService extends AbstractAnalysisResultService {
    private HeapDump renameHeapdump(HeapDump heapDump) {
        ...
        if (!heapDump.heapDumpFile.renameTo(file)) {
            CanaryLog.m10641d("Could not rename heap dump file %s to %s", heapDump.heapDumpFile.getPath(), file.getPath());
        }
        ...
        int max = Math.max(getResources().getInteger(C3940R.integer.leak_canary_max_stored_leaks), 1);
        ...
        if (listFiles.length > max) {
            ...
            if (!listFiles[0].delete()) {
                CanaryLog.m10641d("Could not delete old hprof file %s", listFiles[0].getPath());
            }
        }
        return heapDump2;
    }
    ...
    private boolean saveResult(com.squareup.leakcanary.HeapDump r5, com.squareup.leakcanary.AnalysisResult r6) {
        ...
        L_0x0041:
            ...
            com.squareup.leakcanary.CanaryLog.m10642d(r5, r6, r2)     // Catch:{ all -> 0x003e }
    ...
    public final void onHeapAnalyzed(HeapDump heapDump, AnalysisResult analysisResult) {
        ...
        CanaryLog.m10641d(leakInfo, new Object[0]);
        ...
        if (!z2) {
            string = getString(C3940R.string.leak_canary_no_leak_title);
            i = C3940R.string.leak_canary_no_leak_text;
        } else if (z) {
            ...
            if (analysisResult.failure == null) {
                ...
                string = analysisResult.excludedLeak ? getString(C3940R.string.leak_canary_leak_excluded, new Object[]{classSimpleName, formatShortFileSize}) : getString(C3940R.string.leak_canary_class_has_leaked, new Object[]{classSimpleName, formatShortFileSize});
            } else {
                string = getString(C3940R.string.leak_canary_analysis_failed);
            }
            i = C3940R.string.leak_canary_notification_message;
        } else {
            string = getString(C3940R.string.leak_canary_could_not_save_title);
            i = C3940R.string.leak_canary_could_not_save_text;
        }
